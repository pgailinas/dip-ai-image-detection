...
- **Final Model Selection and Performance Summary** — Comparison of untuned and tuned candidate classifiers during cross-validation-based tuning and validation
...
Performance throughout the experiments was evaluated using Accuracy, Precision, Recall, F1 Score, ROC-AUC, confusion matrices, and ROC curve analysis to assess both threshold-dependent and threshold-independent classifier behavior.
...
Following baseline classifier comparison and hyperparameter exploration, the evaluation focused on the two strongest candidate classifiers: the **Radial Basis Function Support Vector Machine (RBF SVM)** and the **Multi-Layer Perceptron (MLP)**.

Both models were trained using the normalized 26-feature DIP representation and evaluated using stratified cross-validation during the tuning and validation stage. This process was used to compare classifier behavior, assess tuning effectiveness, and identify the strongest candidate models before final independent test evaluation.
...
<p align="center"><em>Table 2: Cross-validation performance comparison of untuned and tuned selected classifiers</em></p>
...
Following cross-validation tuning and validation, the final selected classifiers were evaluated on the fully isolated independent test dataset to assess true generalization performance on unseen data.

<p align="center">
  <img src="images/roc-curve-comparison.png" width="700">
</p>
<p align="center"><em>Figure 2: ROC curve comparison of the final selected classifiers on the independent test set</em></p>

Several important observations emerge from the cross-validation tuning results:

* Hyperparameter tuning improved performance for the **RBF SVM** across all major evaluation metrics, suggesting that nonlinear kernel behavior can be effectively optimized within the DIP feature space.

* Although tuning did not improve the **MLP**, the model still demonstrated strong and stable classification performance, confirming that neural-network-based approaches remain highly effective for modeling the engineered DIP representation.

* Both classifiers demonstrated strong nonlinear modeling capability, reinforcing the observation that the statistical separation between real and AI-generated imagery is not purely linearly separable.

Following tuning and validation, the final selected classifiers were evaluated on the independent test dataset to assess true generalization performance on unseen data.

* The **RBF SVM** achieved the strongest overall ROC-AUC, with its ROC curve remaining above the MLP across most operating regions, indicating more consistent threshold-independent class separability.

* Although the RBF SVM achieved the strongest overall ROC-AUC on the independent test set, the MLP remained highly effective within the DIP feature space and was selected for subsequent feature-level and source-pair analyses because of its strong nonlinear modeling capability across multiple statistical image domains.
...
The full-feature baseline shown in this analysis represents the MLP configuration used consistently throughout the feature-level experiments and evaluated using the same held-out test procedure applied to all feature subsets.
...
<p align="center"><em>Figure 4: Performance comparison between individual feature groups and the full combined model</em></p>
...
* All source-pair experiments were performed using the complete 26-feature DIP representation and the finalized MLP analysis configuration to ensure consistent comparison across dataset combinations.
...
<p align="center"><em>Figure 5: ROC curve comparison across all source-pair experiments</em></p>
