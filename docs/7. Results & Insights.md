...
The relatively strong performance across all three feature groups indicates that discriminative information is broadly distributed throughout the DIP representation rather than concentrated within a single domain. Gradient-domain features capture edge structure and directional behavior, spatial-domain features emphasize texture and entropy characteristics, while frequency-domain features capture spectral energy behavior and frequency irregularities. The full combined model substantially outperformed all isolated feature groups, demonstrating that integrating multiple statistical image domains produces a more robust and generalized AI-image detection representation.
